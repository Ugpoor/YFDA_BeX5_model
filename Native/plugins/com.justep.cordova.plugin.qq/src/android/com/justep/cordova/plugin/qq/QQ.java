...
public class QQ extends CordovaPlugin {
	...
	private static final String QQUSER_ERROR = "QQ User is error";
	private static final String QQUSER_CANCEL = "QQ User is cancelled";
	...
	protected void pluginInitialize() {
		...
		APP_ID = webView.getPreferences().getString(QQ_APP_ID, "222222");
	...
	public boolean execute(String action, final JSONArray args, final CallbackContext callbackContext) throws JSONException {
		...
		// }
		if (action.equals("getParam")) {
			return getParam(callbackContext);
		}
		return super.execute(action, args, callbackContext);
	}

	private boolean getParam(final CallbackContext callbackContext) {
		callbackContext.success("{\"" + QQ_APP_ID + "\":\"" + APP_ID + "\"}");
		return true;
	}
	...
	private boolean ssoLogin(CallbackContext callbackContext) {
		...
		if (mTencent.isSessionValid()) {
			JSONObject jo = makeJson(mTencent.getAccessToken(), mTencent.getOpenId());
			...
			Runnable runnable = new Runnable() {
				...
				@Override
				public void run() {
					mTencent.login(QQ.this.cordova.getActivity(), "all", loginListener);
					// mTencent.loginServerSide(QQ.this.cordova.getActivity(),
					// "all",
					// loginListener);
				}
			};
	...
	private boolean qqshare(JSONArray args, CallbackContext callbackContext) throws JSONException {
		...
		this.cordova.getActivity().runOnUiThread(new Runnable() {
			...
			public void run() {
				...
				mTencent.shareToQQ(QQ.this.cordova.getActivity(), params, qqShareListener);
			}
		});
	...
	private boolean shareToQzone(JSONArray args, CallbackContext callbackContext) throws JSONException {
		...
		this.cordova.getActivity().runOnUiThread(new Runnable() {
			...
			@Override
			public void run() {
				mTencent.shareToQzone(QQ.this.cordova.getActivity(), params, qZoneShareListener);
			}
		});
		return true;
	}
	...
	private boolean addToQQFavorites(JSONArray args, CallbackContext callbackContext) throws JSONException {
		...
		this.cordova.getActivity().runOnUiThread(new Runnable() {
			...
			@Override
			public void run() {
				mTencent.addToQQFavorites(QQ.this.cordova.getActivity(), params, addToQQFavoritesListener);
			}
		});
		return true;
	}
	...
	IUiListener loginListener = new IUiListener() {
		...
		public void onComplete(Object response) {
			if (null == response) {
				QQ.this.webView.sendPluginResult(new PluginResult(PluginResult.Status.ERROR, QQ_RESPONSE_ERROR), currentCallbackContext.getCallbackId());
				return;
			}
			...
			JSONObject jo = makeJson(mTencent.getAccessToken(), mTencent.getOpenId());
			QQ.this.webView.sendPluginResult(new PluginResult(PluginResult.Status.OK, jo), currentCallbackContext.getCallbackId());
		}

		@Override
		public void onError(UiError uiError) {
			QQ.this.webView.sendPluginResult(new PluginResult(PluginResult.Status.ERROR, QQ_LOGIN_ERROR), currentCallbackContext.getCallbackId());
		}

		@Override
		public void onCancel() {
			QQ.this.webView.sendPluginResult(new PluginResult(PluginResult.Status.ERROR, QQ_CANCEL_BY_USER), currentCallbackContext.getCallbackId());
		}
	};
	/**
	 * QQ分享监听
	...
	IUiListener qqShareListener = new IUiListener() {
		...
		public void onComplete(Object response) {
			JSONObject jo = makeJson(mTencent.getAccessToken(), mTencent.getOpenId());
			QQ.this.webView.sendPluginResult(new PluginResult(PluginResult.Status.OK), currentCallbackContext.getCallbackId());
		}

		@Override
		public void onError(UiError e) {
			QQ.this.webView.sendPluginResult(new PluginResult(PluginResult.Status.ERROR, QQ_SHARE_ERROR), currentCallbackContext.getCallbackId());
		}
	...
	IUiListener qZoneShareListener = new IUiListener() {
		...
		@Override
		public void onCancel() {
			QQ.this.webView.sendPluginResult(new PluginResult(PluginResult.Status.ERROR, QZONE_SHARE_CANCEL), currentCallbackContext.getCallbackId());
		}

		@Override
		public void onError(UiError e) {
			QQ.this.webView.sendPluginResult(new PluginResult(PluginResult.Status.ERROR, QZONE_SHARE_ERROR), currentCallbackContext.getCallbackId());
		}
	...
	IUiListener addToQQFavoritesListener = new IUiListener() {
		@Override
		public void onCancel() {
			QQ.this.webView.sendPluginResult(new PluginResult(PluginResult.Status.ERROR, QQFAVORITES_CANCEL), currentCallbackContext.getCallbackId());
		}
		...
		@Override
		public void onError(UiError e) {
			QQ.this.webView.sendPluginResult(new PluginResult(PluginResult.Status.ERROR, QQFAVORITES_ERROR), currentCallbackContext.getCallbackId());
		}
	};
	/**
	 * 取得用户信息监听
	...
	IUiListener getUserInfoListener = new IUiListener() {
		@Override
		public void onCancel() {
			QQ.this.webView.sendPluginResult(new PluginResult(PluginResult.Status.ERROR, QQUSER_CANCEL), currentCallbackContext.getCallbackId());
		}
		...
		public void onComplete(Object response) {
			try {
				...
				e.printStackTrace();
				QQ.this.webView.sendPluginResult(new PluginResult(PluginResult.Status.ERROR, QQUSER_ERROR), currentCallbackContext.getCallbackId());
			}
		}
	...
	private boolean getUserInfo(CallbackContext callbackContext) {
		currentCallbackContext = callbackContext;
		if (mTencent.isSessionValid()) {
			UserInfo info = new UserInfo(cordova.getActivity(), mTencent.getQQToken());
			info.getUserInfo(getUserInfoListener);
			return true;
		} else {
