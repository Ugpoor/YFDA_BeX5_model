...
- (void)scan:(CDVInvokedUrlCommand*)command {
    ...
    [center addObserver:self selector:@selector(sendResult:) name:@"success" object:nil];
    [center addObserver:self selector:@selector(sendResult:) name:@"fail" object:nil];
...
-(void)sendResult:(NSNotification *)resultInfo{
    if ([resultInfo.name  isEqual: @"success"]) {
        CDVPluginResult *result = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsDictionary:resultInfo.userInfo];
...
- (void)returnSuccess:(NSString*)scannedText format:(NSString*)format cancelled:(BOOL)cancelled flipped:(BOOL)flipped callback:(NSString*)callback{
    ...
    if ([resultDict[@"text"] isEqualToString:@""] || [resultDict[@"format"] isEqualToString:@""]) {
...
parentViewController:(UIViewController*)parentViewController
  alterateOverlayXib:(NSString *)alternateXib {
    ...
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(barcodeScanDone) name:@"success" object:nil];
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(barcodeScanDone) name:@"fail" object:nil];
...
//--------------------------------------------------------------------------
- (NSString*)formatStringFrom:(zxing::BarcodeFormat)format {
    if (format == zxing::BarcodeFormat_QR_CODE)      return AVMetadataObjectTypeQRCode;
    if (format == zxing::BarcodeFormat_DATA_MATRIX)  return AVMetadataObjectTypeDataMatrixCode;
    if (format == zxing::BarcodeFormat_UPC_E)        return AVMetadataObjectTypeUPCECode;
    if (format == zxing::BarcodeFormat_EAN_8)        return AVMetadataObjectTypeEAN8Code;
    if (format == zxing::BarcodeFormat_EAN_13)       return AVMetadataObjectTypeEAN13Code;
    if (format == zxing::BarcodeFormat_CODE_128)     return AVMetadataObjectTypeCode128Code;
    if (format == zxing::BarcodeFormat_CODE_39)      return AVMetadataObjectTypeCode39Code;
    if (format == zxing::BarcodeFormat_ITF)          return AVMetadataObjectTypeITF14Code;
    return @"???";
}
...
- (void)loadView {
    ...
    UIView *statusView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, self.view.frame.size.width,rectStatus.size.height)];
    statusView.backgroundColor = [UIColor whiteColor];
    [self.view addSubview:statusView];
    
    //创建一个导航栏
    UINavigationBar *navBar = [[UINavigationBar alloc] initWithFrame:CGRectMake(0, rectStatus.size.height, self.view.frame.size.width, 44)];
    //创建一个导航栏集合
    UINavigationItem *navItem = [[UINavigationItem alloc] initWithTitle:@"二维码/条码"];
    //创建一个右边按钮
    UIBarButtonItem *rightButton = [[UIBarButtonItem alloc] initWithTitle:@"相册" style:UIBarButtonItemStyleDone target:self action:@selector(clickRightButton)];
    ...
    UILabel *lable = [[UILabel alloc]initWithFrame:CGRectMake(0, rectStatus.size.height+navBar.frame.size.height+25, self.view.frame.size.width, 20)];
    lable.text = @"将取景框对准二维码";
...
-(void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary<NSString *,id> *)info{
    ...
    //[self recognizingImage:[self createBitmapRGBA8ContextFromImage:image.CGImage]];
...
-(void)getURLWithImage:(UIImage *)img{
    ...
    if (result) {
        ...
        NSString *format = [self convertZXBarcodeFormat:result.barcodeFormat];
        NSNumber* cancelledNumber = [NSNumber numberWithInt:0];;
        NSMutableDictionary* resultDict = [[NSMutableDictionary alloc] init];
        [resultDict setObject:contents     forKey:@"text"];
        [resultDict setObject:format          forKey:@"format"];
        [resultDict setObject:cancelledNumber forKey:@"cancelled"];
        NSNotification *notice = [NSNotification notificationWithName:@"success" object:nil userInfo:resultDict];
        //[[NSNotificationCenter defaultCenter] postNotificationName:notice object:nil];
        [[NSNotificationCenter defaultCenter] postNotification:notice];
    } else {
        NSNotification *notice = [NSNotification notificationWithName:@"fail" object:nil];
        [[NSNotificationCenter defaultCenter] postNotification:notice];
    }
}
